"""
ComBat Pipeline — Supervised (Train + Test)
==========================================================
...
Parameters are saved as a single H5 file:
    combat_params.h5
        /meta/label_map         str   (n_categories, 2)  [[code, name], ...]
...
OUT_TRAIN_NORM = "/shared/home/rakhat.myrzakhan/new_combat/sup/train_normalized.h5"
OUT_TEST_NORM  = "/shared/home/rakhat.myrzakhan/new_combat/sup/test_normalized.h5"
OUT_TEST_RAW   = "/shared/home/rakhat.myrzakhan/new_combat/sup/test_raw.h5"
OUT_PARAMS_H5  = "/shared/home/rakhat.myrzakhan/new_combat/sup/combat_params.h5"
...
def load_labels():
    ...
    print(f"  Train : {len(train):,} samples | "
          f"{train[COL_BATCH].nunique():,} batches | "
          f"{train[COL_CATEGORY].nunique():,} categories")
    print(f"  Test  : {len(test):,} samples  | "
          f"{test[COL_BATCH].nunique():,} batches | "
          f"{test[COL_CATEGORY].nunique():,} categories")
    return train, test
...
def build_covariates(labels_df: pd.DataFrame, disease_encoder=None):
    covars = pd.DataFrame(index=labels_df.index)
    covars["batch"] = labels_df[COL_BATCH].values

    if disease_encoder is None:
        cat = pd.Categorical(labels_df[COL_CATEGORY])
    else:
        cat = pd.Categorical(labels_df[COL_CATEGORY],
                             categories=disease_encoder.categories)

    covars["disease"] = cat.codes.astype(int)
    return covars, cat


# ══════════════════════════════════════════════════════════════════════════════
#  STEP 3 — FIT ComBat ON TRAIN
# ══════════════════════════════════════════════════════════════════════════════

def fit_combat_chunked(expr_df: pd.DataFrame, covars_df: pd.DataFrame):
    section(f"STEP 3 — Fit ComBat on train  "
    ...
    if n_ok == 0:
        raise RuntimeError("ComBat failed for every gene chunk.")
...
def save_h5(expr_df, labels_df, gene_names, path, tag):
    ...
    data_out  = expr_df.values.T.astype(np.float32)
...
# ══════════════════════════════════════════════════════════════════════════════
#  SAVE PARAMS — single H5 file, one group per chunk
# ══════════════════════════════════════════════════════════════════════════════

def save_params(chunk_info: list, disease_encoder, path: str):
    """
    Save all ComBat parameters into a single H5 file.

    Layout:
        /meta/label_map     str  (n_cat, 2)   [["0","Z"], ["1","C_Blood"], ...]
        /meta/settings      str  (4, 2)        [["chunk_size","2000"], ...]
        /chunk_0/
            slice_start     int scalar
            slice_stop      int scalar
            ...
            gamma_hat       float64  (n_batches, n_good)   if has_estimates
            delta_hat       ...
            ...             (all ESTIMATE_KEYS, compressed)
        /chunk_1/ ...
    """
    os.makedirs(os.path.dirname(path) or ".", exist_ok=True)
    dt = h5py.special_dtype(vlen=str)
    ...
    with h5py.File(path, "w") as f:
        ...
        label_map_arr = np.array(
            [[str(i), c] for i, c in enumerate(disease_encoder.categories)],
            dtype=object,
        )
        meta_grp.create_dataset("label_map", data=label_map_arr.astype("S"))
        ...
        for idx, c in enumerate(tqdm(chunk_info, desc="  Saving chunks")):
            ...
            for k in ESTIMATE_KEYS:
                ...
                if k == "batches":
                    # Store batch strings as fixed-length byte strings
                    batch_arr = np.array(
    ...
    size_mb = os.path.getsize(path) / 1024 ** 2
    print(f"\n  Saved combat_params.h5  →  {path}  ({size_mb:.1f} MB)")
...
def load_params(path: str):
    """
    Reconstruct chunk_info and label_map from combat_params.h5.

    Returns
    -------
    chunk_info : list[dict]   {slice, good_rows, estimates}
    label_map  : dict         {int_code -> category_string}
    """
    section("Load saved ComBat parameters")

    with h5py.File(path, "r") as f:

        # ── /meta ─────────────────────────────────────────────────────────────
        label_map_raw = f["meta/label_map"][:]
        label_map = {
            int(row[0].decode("utf-8") if isinstance(row[0], bytes) else row[0]):
...
if __name__ == "__main__":
    ...
    # 3. Covariates
    train_covars, disease_encoder = build_covariates(train_lbl)
    test_covars,  _               = build_covariates(test_lbl, disease_encoder)

    # 4. Fit ComBat on train
    ...
    save_params(chunk_info, disease_encoder, OUT_PARAMS_H5)
